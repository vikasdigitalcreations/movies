...
| `src-tauri/src/bin/probe.rs` | Developer probe. `survey` counts playable titles, `chain` walks the app's own three-tier failover per title and says where it stops, `mirror` decodes a stream's CloudFront policy, `fourkplay`/`fourkmirrors` separate a dead source from a stale resolver, `addons` checks the id bridge |
