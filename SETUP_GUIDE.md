...
The vendored crate is already in `vendor/moviebox-tui` (MovieBox-Tui v0.1.22, unmodified -- our copy is byte-identical to upstream, so a refresh is a plain tree replacement). Upstream ships fixes for provider changes every few weeks, so check for a newer tag whenever playback breaks. To refresh it:

```bash
git clone --depth 1 --branch v0.1.22 https://github.com/mesamirh/MovieBox-Tui vendor/moviebox-tui
```
